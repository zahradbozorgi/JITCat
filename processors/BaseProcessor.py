...
class BaseProcessor:
    def __init__(self, dataset_name, use_encoding, use_bucketing, num_nearest_neighbors, distance_metric):
        ...
        elif distance_metric == "euclidean_exponential":
            self.distance_metric = self.euclidean_exponential
        elif distance_metric == "cosine_exponential":
            self.distance_metric = self.cosine_exponential
        elif distance_metric == "dtw_exponential":
            self.distance_metric = self.dtw_exponential
    ...
    def dtw_exponential(self, x, y):
        return np.exp(-dtw.distance(x, y))
    
    def euclidean_exponential(self, x, y):
        return np.exp(-np.linalg.norm(x-y))
    
    def cosine_exponential(self, x, y):
